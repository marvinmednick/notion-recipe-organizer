...
class ConfigLoader:
    ...
    def format_conflict_rules_for_prompt(self) -> str:
        """Format conflict resolution rules for prompt inclusion."""
        rules = self.load_conflict_rules()
        formatted = []

        # Primary category precedence
        precedence = rules.get("primary_category_rules", {}).get("precedence_order", {})
        if precedence:
            formatted.append(
                "**Primary Category Precedence (higher precedence wins conflicts):**"
            )
            for level, categories in precedence.items():
                if isinstance(categories, list):
                    formatted.append(f"  {level}. {', '.join(categories)}")
                else:
                    formatted.append(f"  {level}. {categories}")

        # Specific rules
        specific_rules = rules.get("primary_category_rules", {}).get(
            "specific_rules", {}
        )
        if specific_rules:
            formatted.append("\n**Specific Conflict Rules:**")
            for rule_name, rule_info in specific_rules.items():
                description = rule_info.get("description", "")
                reasoning = rule_info.get("reasoning", "")
                formatted.append(
                    f"- **{rule_name.replace('_', ' ').title()}**: {description}"
                )
                if reasoning:
